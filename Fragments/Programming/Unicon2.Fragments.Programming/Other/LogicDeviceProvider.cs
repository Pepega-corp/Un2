...
namespace Unicon2.Fragments.Programming.Other
{
    public class LogicDeviceProvider
    {
        ...
        public void SetDeviceContext(DeviceContext deviceContext)
        {
            ...
            this._fileDriver.SetDataProvider(deviceContext.DataProviderContainer);
        }
        ...
        public async Task WriteLogicArchive(byte[] logicPjectBytes, bool hasFileSystem, ushort writeDataLen = 64)
        {
            ...
            else
            {
                ...
                var isWrote = await Write(writeArchData, LOGIC_ARCH_START_ADDRESS_OLD_DEVICES, "WriteLogicArchive",
                    writeDataLen);
                if (!isWrote)
                {
        ...
        public async Task WriteLogicProgrammBin(ushort[] logbin, bool hasFileSystem, ushort writeDataLen = 64)
        {
            ...
            ushort writeStartAddress =
                hasFileSystem ? LOGIC_BIN_START_ADDRESS_NEW_DEVICES : LOGIC_BIN_START_ADDRESS_OLD_DEVICES;
        ...
        private async Task<bool> Write(ushort[] values, ushort startAddress, string writeQueryName, ushort writeDataLen)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var programmSaveResult = await this.DeviceContext.DataProviderContainer.DataProvider.OnSuccessAsync(
                    async (provider) =>
                        Result<IQueryResult>.Create(
                            await provider.WriteMultipleRegistersAsync((ushort) (startAddress + i * writeDataLen), val,
                                $"{writeQueryName}{i}"), true));

                if (!programmSaveResult.IsSuccess || !programmSaveResult.Item.IsSuccessful)
                {
                    return false;
                }
            }
        ...
        public async Task WriteStartlogicProgrammSignal(bool hasFileSystem)
        {
            ...
            var res = await this.DeviceContext.DataProviderContainer.DataProvider.OnSuccessAsync(
                async (provider) =>
                    Result<IQueryResult>.Create(
                        await provider.WriteSingleCoilAsync(startAddress, true, "StartLogicProgramm"), true));
        ...
        {
            var startAddress = hasFileSystem
                ? LOGIC_SYGNAL_START_ADDRESS_NEW_DEVICES
                : LOGIC_SYGNAL_START_ADDRESS_OLD_DEVICES;

            var res = await this.DeviceContext.DataProviderContainer.DataProvider.OnSuccessAsync(
                async (provider) =>
                    Result<IQueryResult<ushort[]>>.Create(
                        await provider.ReadHoldingResgistersAsync(startAddress, (ushort) connectionsCount,
                            "ReadLogProgSignals"), true));
        ...
        private async Task<byte[]> ReadLogicArchive(ushort readDataLen)
        {
            ...
            for (; i < count; i++)
            {
                var res = await this.DeviceContext.DataProviderContainer.DataProvider.OnSuccessAsync(
                    async (provider) =>
                        Result<IQueryResult<ushort[]>>.Create(
                            await provider.ReadHoldingResgistersAsync(address, readDataLen, $"ReadLogicArcive{i}"),
                            true));


                if (res.IsSuccess && res.Item.IsSuccessful)
                {
                    logicArchive.AddRange(res.Item.Result);
                }
                else
                {
            ...
            if (residue > 0)
            {
                var res = await this.DeviceContext.DataProviderContainer.DataProvider.OnSuccessAsync(
                    async (provider) =>
                        Result<IQueryResult<ushort[]>>.Create(
                            await provider.ReadHoldingResgistersAsync(address, (ushort) residue, $"ReadLogicArcive{i}"),
                            true));
