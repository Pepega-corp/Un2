...
namespace NModbus4.UnitTests.Utility
{
    public class ModbusUtilityFixture
    {
        ...
        {
            Assert.Equal<byte[]>(new byte[] { }, ModbusUtility.GetAsciiBytes(new byte[] { }));
            Assert.Equal<byte[]>(new byte[] { }, ModbusUtility.GetAsciiBytes(new ushort[] { }));
        }
        ...
        {
            Assert.Equal<byte[]>(new byte[] { 255 }, ModbusUtility.HexToBytes("FF"));
        }

        [Fact]
        public void HexToBytes2()
        {
            Assert.Equal<byte[]>(new byte[] { 204, 255 }, ModbusUtility.HexToBytes("CCFF"));
        }

        [Fact]
        public void HexToBytesEmpty()
        {
            Assert.Equal<byte[]>(new byte[] { }, ModbusUtility.HexToBytes(string.Empty));
        }
        ...
        {
            Assert.Equal<byte[]>(new byte[] { 255, 255 }, ModbusUtility.CalculateCrc(new byte[] { }));
        }
        ...
        {
            Assert.Equal<ushort[]>(new ushort[] { 1, 2 }, ModbusUtility.NetworkBytesToHostUInt16(new byte[] { 0, 1, 0, 2 }));
        }
        ...
        {
            Assert.Equal<ushort[]>(new ushort[] { }, ModbusUtility.NetworkBytesToHostUInt16(new byte[] { }));
        }
